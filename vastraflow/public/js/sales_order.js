...
vastraflow.apply_link_filters = function (frm, config) {
	...
	// Custom queries (not just filters) so the dropdown sorts by how often each
	// item is actually used on a garment order, most-used first - not alphabetical.
	frm.set_query("fabric", () => ({
		query: "vastraflow.apparel_core.api.fabric_link_query",
		filters: filters.fabric || {},
	}));
	frm.set_query("collar_type", () => ({
		query: "vastraflow.apparel_core.api.collar_link_query",
		filters: filters.collar_type || {},
	}));
	...
	frm.set_query("item_code", "items", () => {
		return frm.doc.is_garment_order
			? { query: "vastraflow.apparel_core.api.product_link_query", filters: filters.product_type || {} }
			: {};
	});
};
