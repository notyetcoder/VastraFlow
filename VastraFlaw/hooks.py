...
app_description = (
	"VastraFlow ERP - Order Book and Price List for apparel manufacturing. "
...
# NOTE: VastraFlow Order Book's client script is auto-loaded by the
# Frappe framework because it lives alongside the DocType's own folder
# (apparel_core/doctype/vastraflow_order_book/). It does NOT need to also
...
# VastraFlow its own dedicated icon separate from ERPNext's standard
# modules, landing straight on the Order Book list when tapped.
add_to_apps_screen = [
	{
		"name": "p3erp",
		"logo": "/assets/p3erp/images/vastraflow_logo.png",
		"title": "VastraFlow",
		"route": "/app/vastraflow-order-book",
	}
]

# NOTE: VastraFlow Order Book has no doc_events registered here. Its
# validate/on_submit/on_cancel logic lives directly in its own controller
# (apparel_core/doctype/vastraflow_order_book/vastraflow_order_book.py).
