...
def _ensure_variant(code: str, template: str, attribute: str, value: str, group: str, uom: str, image: str | None = None):
	...
	item = frappe.new_doc("Item")
	item.item_code = code
	# The bare attribute value ("Dotnet", "Round Neck") - not "FB - Dotnet". Item
	# pickers show this (item_name, via title_field) as the bold primary text
	# instead of the code, since users pick fabrics/collars by name, not code - a
	# template prefix baked into every name would defeat that entirely.
