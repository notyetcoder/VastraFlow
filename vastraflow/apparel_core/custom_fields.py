...
# - for these combinations, and only needs its own Colour choice when it is NOT
# locked. Front & Back locks both, leaving only sleeve open on its own. Full
# sublimation locks all three, including sleeve.
...
# Front/Back/Sleeve Colour stay visible at all times (never hidden) - only *blocked
...
# Sleeve is also locked under Full sublimation (the print already showed "Sublimation"
# for it in that case - the form now matches that instead of silently accepting a
# colour the print was always going to ignore).
SLEEVE_LOCKED_RO = (
	f"eval:doc.sleeve_type=='Sleeveless' || doc.sublimation_type=='Full sublimation' "
	f"|| !{ANY_SUBLIMATION}.includes(doc.sublimation_type)"
)
...
def get_custom_fields() -> dict:
	return {
		"Sales Order": [
			...
			{
				"fieldname": "vf_fabric_collar_column",
			...
			},
			{
				...
				"fieldtype": "Link",
				"options": "Item",
				"insert_after": "vf_fabric_collar_column",
				"depends_on": GARMENT,
			...
			},
			{
				...
				"depends_on": GARMENT,
				"description": (
					"All six always show - Front/Back/Sleeve grey out (blocked, not hidden) "
					"when the main Sublimation Type already covers them: Full sublimation "
					"blocks all three, Front/Back/Front & Back sublimation block just that "
					"panel. Border and Collar Colour are always free to set. Colour choices "
					"are print-only - they never affect pricing or the BOM."
				),
			},
			{
			...
			{
				# A genuine new row (Section Break) is safe here: every field in this
				# row is always visible (read-only when locked, never hidden), so
				# nothing can be left stranded alone the way Border used to be on a
				# Sleeveless order, before that bug was fixed.
			...
			{
				# Border is a trim colour, not a print panel - always free to set,
				# never locked.
