...
from vastraflaw.apparel_core import pricing
...
class vastraflawOrderBook(Document):
	"""vastraflaw Order Book (vastraflaw Order Book) - a lightweight custom order-entry layer
	...
	Lifecycle:
	  ...
	  Submit  -> docstatus 1. Fabric + Sublimation Type (the only two
	             fields that affect price - see pricing.py) must resolve
	             to a P3 Price List rate, or submit is blocked - unless
	             the current user has the Account Manager role, in which
	             case a quick-entry dialog lets them define the missing
	             price inline. Creates a real (draft) Sales Order with
	             ONE line item PER matrix cell - never merged, even
	             though every cell on this order shares the exact same
	             rate - links it back via `sales_order`.
	...
	on this form - see P3 Price List / pricing.py for why pricing lives
	in its own dedicated, Account-Manager-controlled system instead.
	Only Fabric + Sublimation Type feed into it; Collar Type, Stitching
	Type, Size, Sleeve Type and Button Qty are production specs only and
	never affect the rate.
	"""
	...
	def before_submit(self):
		...
		if result["missing_base_price"]:
			frappe.throw(
				_(
					"Cannot submit - no price found for Fabric '{0}' + Sublimation '{1}'."
					"<br><br>{2}"
				).format(
					self.fabric_label or self.fabric,
					self.sublimation_type,
					_("Ask an Account Manager to add it to the Price List, or use the "
	...
	# -- pricing --------------------------------------------------------
	#
	# Fabric + Sublimation Type are the ONLY two fields that affect price
	# (see pricing.py / P3 Price List). Both are fixed for the whole
	# order, so every matrix cell shares the exact same rate - there is
	# no per-cell pricing anymore. Collar Type, Stitching Type, Size,
	# Sleeve Type and Button Qty are production specs only.

	def get_price(self):
		"""Full pricing.calculate_price() result for this order. Cached on
		the instance for the duration of one request/transaction so
		before_submit and create_linked_sales_order never call it twice
		with different results mid-flight.
		"""
		if not hasattr(self, "_price_result"):
			self._price_result = pricing.calculate_price(
				fabric=self.fabric,
	...
	def create_linked_sales_order(self):
		...
		if result["missing_base_price"]:
			...
			frappe.throw(
				_("No price found for Fabric '{0}' + Sublimation '{1}'.").format(
					self.fabric_label or self.fabric, self.sublimation_type
				)
			)
	...
	def create_sales_order_from_book(self):
		...
		"""
		if self.docstatus != 1:
			frappe.throw(_("Submit this vastraflaw Order Book first."))
	...
	def get_missing_price_prefill(self):
		...
		flow: returns field values pre-formatted to seed a new P3 Price
		List quick-entry dialog for this order's exact Fabric +
		Sublimation Type, if that combination doesn't already have a
		price. Returns None if pricing already resolves (nothing to add).
		"""
		if "Account Manager" not in frappe.get_roles():
			frappe.throw(_("Only an Account Manager can add pricing."))

		result = self.get_price()
		if not result["missing_base_price"]:
			return None
