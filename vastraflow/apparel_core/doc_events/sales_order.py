...
def _normalize_panels(doc):
	"""Clear Front/Back/Sleeve Colour when the panel is locked, so a colour picked
	before Sublimation Type (or Sleeve Type) changed to lock it is never carried
	forward or printed. The client only *blocks editing* of a locked panel
	(read_only_depends_on) rather than hiding it, so a value set before the lock
	kicked in would otherwise survive untouched - this is what actually clears it.

	Border Colour and Collar Colour are never locked (see custom_fields.py) - just
	plain optional colours, nothing to normalize here."""
	...
	# Sleeve is also locked under Full sublimation - the print always showed
	# "Sublimation" for it in that case, so a colour typed there was already being
	# silently ignored. Locking it here on the form now matches that.
	sleeve_open = (
		sublimation in _ANY_SUBLIMATION
		and doc.get("sleeve_type") != "Sleeveless"
		and sublimation != "Full sublimation"
	)

	for open_, colour_field in (
		(front_open, "front_colour"),
		(back_open, "back_colour"),
		(sleeve_open, "sleeve_colour"),
	):
		if not open_:
