...
// vastraflaw.py - deliberately NOT an exact-string map, since real
...
frappe.ui.form.on('vastraflaw Order Book', {
	setup(frm) {
	...
	// already states exactly what's missing. This is purely a
	// client-side convenience so an Account Manager doesn't have to fail
	// a submit, go find the Price List, and come back - it opens
		...
		return frm.call('get_missing_price_prefill').then(r => {
			...
			return new Promise((resolve, reject) => {
				frappe.msgprint({
					...
					message: __('No price found for Fabric "{0}" + Sublimation "{1}". Add one now, then click Submit again.', [frm.doc.fabric_label || frm.doc.fabric, frm.doc.sublimation_type]),
					indicator: 'orange'
				});
				frappe.new_doc('P3 Price List', prefill);
